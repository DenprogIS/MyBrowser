...
namespace Browser
{
    public partial class Form1 : Form
    {
        ...
        private void showDialogsInList()
        {
            ...
            ArrayList UnreadMessagesFromUser = new ArrayList();
            ...
            for (int i = 1; i < count+1; i++)
            {
                ...
                if (title == " ... ")
                    usersDialogIDs.Add(user);
                else
                    listBox1.Items.Add(title);
                //if (read_state == 0)

            }
            string userInfo, firstName, lastName, postfix;
            for (int i = 0; i < usersDialogIDs.Count; i++)
            {
                userInfo = GET("https://api.vk.com/method/users.get?user_ids=" + (usersDialogIDs[i] as Tuple<int,int>).Item1);
                ...
                if ((usersDialogIDs[i] as Tuple<int, int>).Item2==1)
